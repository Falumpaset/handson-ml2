 ...
 * per pixel are only read from the global header. For some GIFs, local palettes change this to a typically larger
 * value. To be certain to get the correct color depth, call setDetermineImageNumber(true) before calling check(). The
 ...
 * Use {@link #setDetermineImageNumber} with <code>true</code> as argument to identify animated GIFs ({@link
 ...
 * That new method lets the user specify whether textual comments are to be stored in an internal list when encountered
 * in an input image file / stream. Added two methods to return the physical width and height of the image in dpi:
 * {@link #getPhysicalWidthDpi()} and {@link #getPhysicalHeightDpi()}. If the physical resolution could not be
 * retrieved, these methods return <code>-1</code>.
 * </li>
 ...
 * Changed checkJpeg() so that other APP markers than APP0 will not lead to a failure anymore. Released as 1.3.</li>
 * <li><strong>2003-07-28</strong> Bug fix - skip method now takes return values into consideration.
 * Less bytes than necessary may have been skipped, leading to flaws in the retrieved information in some cases. Thanks
 * to Bernard Bernstein for pointing that out. Released as 1.4.</li>
 * <li><strong>2004-02-29</strong> Added support for recognizing progressive JPEG and
 * interlaced PNG and GIF. A new method {@link #isProgressive()} returns whether ImageInfo has found that the storage
 * type is progressive (or interlaced). Thanks to Joe Germuska for suggesting the feature. Bug fix: BMP physical
 ...
 * (interlaced in GIF terminology) did not work (thanks to Franz Jeitler for pointing this out). Now it should work, but
 * only if the number of images is determined. This is because information on interlacing is stored in a local image
 * header. In theory, different images could be stored interlaced and non-interlaced in one file. However, I think
 * that's unlikely. Right now, the last image in the GIF file that is examined by ImageInfo is used for the
 * "progressive" status.</li>
 * <li><strong>2005-01-02</strong> Some code clean up (unused methods and variables
 * commented out, missing javadoc comments, etc.). Thanks to George Sexton for a long list. Removed usage of
 * Boolean.toString because it's a Java 1.4+ feature (thanks to Gregor Dupont). Changed delimiter character in compact
 * output from semicolon to tabulator (for better integration with cut(1) and other Unix tools). Added some points to
 * the <a href="http://schmidt.devlib.org/image-info/index.html#knownissues">'Known issues' section of the website</a>.
 * Released as 1.6.</li>
 * <li><strong>2005-07-26</strong> Removed code to identify Flash (SWF) files.
 * Has repeatedly led to problems and support requests, and I don't know the format and don't have the time and interest
 * to fix it myself. I repeatedly included fixes by others which didn't work for some people. I give up on SWF. Please
 * do not contact me about it anymore. Set package of ImageInfo class to org.devlib.schmidt.imageinfo (a package was
 * repeatedly requested by some users). Released as 1.7.</li>
 * <li><strong>2006-02-23</strong> Removed Flash helper methods which weren't used elsewhere.
 * Updated skip method which tries "read" whenever "skip(Bytes)" returns a result of 0. The old method didn't work with
 * certain input stream types on truncated data streams. Thanks to Martin Leidig for reporting this and sending in code.
 * Released as 1.8.</li>
 * </li>
 * <li><strong>2006-11-13</strong> Removed check that made ImageInfo report JPEG APPx
 * markers smaller than 14 bytes as files in unknown format. Such JPEGs seem to be generated by Google's Picasa
 * application. First reported with fix by Karl von Randow. Released as 1.9.</li>
 * </ul>
 *
...
/*	public static final int COLOR_TYPE_UNKNOWN = -1;
    ...
    /**
     * The names of all supported file formats. The FORMAT_xyz int constants can be used as index values for this
     * array.
     */
    ...
    /**
     * Call this method after you have provided an input stream or file using {@link #setInput(InputStream)} or {@link
     * #setInput(DataInput)}. If true is returned, the file format was known and information on the file's content can
     ...
     * from the input image stream / file. Any number &gt;= 0 and smaller than this number of comments is then a valid
